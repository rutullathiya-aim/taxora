<div>
    ...
    <x-slot:breadcrumbs>
        <x-breadcrumbs :links="['Clients' => route('clients.index'), 'Trash']" />
    </x-slot:breadcrumbs>
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 xl:grid-cols-4 gap-5 my-6">
        <flux:card>
            <div class="flex items-center gap-4">
                ...
                <div>
                    <flux:heading>Deleted Clients</flux:heading>
    ...
    <flux:card>
        <div class="flex flex-col md:flex-row md:items-center gap-4">
            <div class="w-full md:w-72">
                <flux:input wire:model.live.debounce.300ms="search" placeholder="Search deleted clients..."
                    icon="magnifying-glass" />
            </div>

            <div class="w-full md:w-auto md:ml-auto flex items-center gap-2">
                <flux:button variant="subtle" href="{{ route('clients.index') }}" wire:navigate icon="arrow-left"
                    class="w-full md:w-auto">Back to Clients</flux:button>
            </div>
        </div>
            ...
            <flux:table.rows>
                @forelse($clients as $index => $client)
                    <flux:table.row>
                        ...
                        <flux:table.cell>{{ $client->deleted_at->format('d M Y H:i') }}</flux:table.cell>

                        <flux:table.cell align="center">
                            <flux:dropdown position="bottom" align="end">
                                <flux:button variant="ghost" size="sm" icon="ellipsis-vertical" inset="top bottom" />
                                <flux:menu>
                                    <flux:menu.item icon="arrow-path" wire:click="restoreClient('{{ $client->id }}')">
                                        Restore
                                    </flux:menu.item>
                            ...
                            <flux:modal :name="'delete-client-'.$client->id" class="min-w-[22rem]">
                                <form wire:submit="forceDeleteClient('{{ $client->id }}')">
                                    <div class="space-y-6">
                                        <div>
                                            <flux:heading size="lg">Delete Client Permanently?</flux:heading>
                                            <flux:text class="text-sm text-zinc-500 mt-2">
                                                Are you sure you want to permanently delete
                                                <b>{{ $client->client_name }}</b>? This action cannot be undone and all
